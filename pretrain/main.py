...
def pre_train_one_ep(ep, args: arg_util.Args, tb_lg: misc.TensorboardLogger,
                     itrt_train, iters_train, model: DistributedDataParallel,
                     optimizer, scaler: torch.cuda.amp.GradScaler):
    model.train()
    ...
    for it, inp in enumerate(me.log_every(iters_train, itrt_train, 3, header)):
        ...
        min_lr, max_lr, min_wd, max_wd = lr_wd_annealing(
            optimizer, args.lr, args.wd, args.wde,
            it + ep * iters_train, args.wp_ep * iters_train, args.ep * iters_train
        )

        inp = inp.to(args.device, non_blocking=True)

        with torch.cuda.amp.autocast(dtype=torch.half,enabled=True):
